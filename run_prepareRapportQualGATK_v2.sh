...
function usage
{
    ...
    echo -e "\nEXAMPLE: ./run_prepareRapportQualGATK_v2.sh -i /storage/IN/111125_HWUSI-EAS1884_00002_FC64F86AAXX/BWA-GATK/DEPTH -o /storage/IN/111125_HWUSI-EAS1884_00002_FC64F86AAXX/JoinRapport -bed /storage/IN/Reference/Capture/MMR/DiagCapture-11genes_20130730.bed"
}
...
do
	...
	do
		...
		pos1=$(($pos1True + 1));
		pos2=$pos2True;
		linePos1=$(grep -w -n "$chro	$pos1" $fileDepth | awk -F":" '{print $1}');
		linePos2=$(grep -w -n "$chro	$pos2" $fileDepth | awk -F":" '{print $1}');
		linePos1True=$linePos1;
		linePos2True=$linePos2;
		#Init pour chaque patient
		SumDepthInterval=null
		MeanDepthInterval=null
		MinDepthInterval=null
		...
		then
			...
			MeanDepthInterval=0;
			MinDepthInterval=0;
			#~ echo -e "\tb1	$ind	$linePos1	$interval:	$MinDepthInterval"
		elif [ $MinDepthInterval = 0 ]
		then
			...
			#~ echo -e "\tb2	$ind	$linePos1	$interval:	$MinDepthInterval"
		else
			NombreLineDepth=$(($linePos2 - $linePos1 + 1));
			...
			MeanDepthInterval=$(echo "scale = 2; $SumDepthInterval/$taille" | bc);
			#MinInterval
			if [ $NombreLineDepth -lt $taille ] #Si le nombre de ligne du fichier depth est inférieur à la taille de la cible dans le bed-> c'est qu'il y a des positions a 0x (=taille min)
			then
				echo "linePos1 linePos2 : $linePos2 - $linePos1 + 1"
				echo "NbreLine < Taille : $NombreLineDepth < $taille"
				MinDepthInterval=0
			else
			...
			#~ echo -e "\tb3	$ind	$linePos1	$interval:	$MinDepthInterval"
		fi
		echo "$ind	$gene	$exon	$taille	$SumDepthInterval	$MeanDepthInterval	$MinDepthInterval" >> $outputdir"/QualityGATK.txt";
	done
